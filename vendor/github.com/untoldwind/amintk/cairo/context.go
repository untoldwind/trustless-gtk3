...
var ContextType = glib.Type(C.cairo_gobject_context_get_type())
...
func ContextNew(target *Surface) *Context {
...
// SetSourcePixbuf is a wrapper around gdk_cairo_set_source_pixbuf().
func (v *Context) SetSourcePixbuf(pixbuf *gdk.Pixbuf, pixbufX, pixbufY float64) {
	ptr := (*C.GdkPixbuf)(pixbuf.Native())
	C.gdk_cairo_set_source_pixbuf(v.native(), ptr, C.gdouble(pixbufX), C.gdouble(pixbufY))
}
...
type CallbackContextBoolean func(context *Context) bool

func (c CallbackContextBoolean) Call(args []glib.Value) *glib.Value {
	var arg0 *Context
	var arg0Ok bool
	for _, value := range args {
		if actual, fundamental := value.Type(); actual == ContextType && fundamental == glib.TypeBoxed {
	...
	if !arg0Ok {
		fmt.Fprintln(os.Stderr, "WARNING: CallbackContextBoolean: No context found in args")
	}
	res := c(arg0)
